...
function createDownloadsController({ getMainWindow, getTabManager, ipcMain, log }) {
  ...
  const getPersistPath = () => {
    try {
      ...
      return path.join(process.cwd(), 'downloads.json');
    }
  };

  const loadPersistedDownloads = () => {
    ...
    try {
      if (fs.existsSync(file)) {
        ...
        if (Array.isArray(parsed)) {
          downloadsStore = parsed;
        }
      }
    } catch (err) {
      console.warn('[Downloads] Failed to load persisted downloads', err);
    }
  };
  ...
  const persistDownloads = () => {
    const file = getPersistPath();
    try {
      fs.writeFileSync(file, JSON.stringify(downloadsStore, null, 2), 'utf-8');
    } catch (err) {
      console.warn('[Downloads] Failed to persist downloads', err);
    }
  };
  ...
  const recordManualDownload = ({ fileName, savePath, sourceUrl = '', mimeType = '', totalBytes = 0 }) => {
    const id = ++downloadCounter;
    const now = Date.now();
    upsertDownloadEntry(id, {
      url: sourceUrl,
  ...
  const registerIpcHandlers = () => {
    ...
    ipcMain.handle('downloads:show-in-folder', (_e, id) => {
      ...
      // fallback: open downloads directory if specific path missing
      const baseDir = lastDownloadDirectory || app.getPath('downloads');
      try {
